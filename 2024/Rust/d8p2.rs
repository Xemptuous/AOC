    ...
    for (_, antennas) in antenna_locations.iter() {
        ...
        for a1 in antennas {
            for a2 in antennas.iter().skip(1) {
                ...
                // antenna location is also counted
                antinode_grid[a1.index] = b'#';
                antinode_grid[a2.index] = b'#';
                ...
                let mut dx_a1 = dx;
                let mut dy_a1 = dy;
                while let Some(i) = get_antinode_position(a1, dx_a1, dy_a1, w, h) {
                    antinode_grid[i] = b'#';
                    dx_a1 += dx;
                    dy_a1 += dy;
                }
                ...
                let mut dx_a2 = dx;
                let mut dy_a2 = dy;
                while let Some(i) = get_antinode_position(a2, -dx_a2, -dy_a2, w, h) {
                    antinode_grid[i] = b'#';
                    dx_a2 += dx;
                    dy_a2 += dy;
                }
